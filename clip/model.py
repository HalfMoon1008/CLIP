...
# AttentionPool2d 클래스는 주로 이미지에서 중요한 부분에 집중하기 위해 사용
# multi_head_forward()를 통해 전통적인 pooling과 달리 q, k, v와 head 개수까지 중요한 공간적 특징을 선택적으로 집약
# class token: Vision Transformer에서의 CLS 토큰과 유사하게, 이미지의 전반적인 특징을 대표하는 벡터를 생성
# projection layer: Attention 메커니즘 내에서 Key, Query, Value를 생성하고 최종 출력을 원하는 차원으로 변환
# 이 클래스는 자원 할당을 조정하여 특정 공간적 정보를 강화하는 방법을 제공
class AttentionPool2d(nn.Module):
    def __init__(self, spacial_dim: int, embed_dim: int, num_heads: int, output_dim: int = None):
        ...
        super().__init__()
        self.positional_embedding = nn.Parameter(torch.randn(spacial_dim ** 2 + 1, embed_dim) / embed_dim ** 0.5) # 위치정보를추가하이 위한 파라미터, special_dim의 제곱 +1의 크기로 초기화된다. 추가된 +1은 클래스 토큰을 위한 공간
        self.k_proj = nn.Linear(embed_dim, embed_dim) # K
        self.q_proj = nn.Linear(embed_dim, embed_dim) # Q
        self.v_proj = nn.Linear(embed_dim, embed_dim) # V
        self.c_proj = nn.Linear(embed_dim, output_dim or embed_dim) # Attention 출력을 원하는 차원으로 변환되는 레이어
        self.num_heads = num_heads # Multi-Head Attention에서 사용하는 헤드 수를 저장
